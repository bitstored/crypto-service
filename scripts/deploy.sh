...
docker build --tag=gcr.io/${PROJECT}/crypto-service .
docker push gcr.io/${PROJECT}/crypto-service

# Optional: Remove other old images automatically if not needed
# prev_ver=`gcloud container images list-tags gcr.io/${PROJECT}/crypto-service | grep -v latest | awk '{if(NR>1)print $1}'`
# if [ ! -z "$prev_ver" ]; then
#     yes | gcloud container images delete gcr.io/${PROJECT}/crypto-service@sha256:${prev_ver} --force-delete-tags
# fi
...
# kubectl expose deployment crypto-service --type=LoadBalancer --port 4001 --target-port 4001
